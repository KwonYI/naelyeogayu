...
public interface ProductDao extends JpaRepository<Product, Long> {
	...
	@Query(value = "SELECT * FROM product WHERE CATEGORY = :category ORDER BY STATUS AND END_DATE ASC LIMIT :limit, :CONTENT_CNT", nativeQuery = true)
	...
	@Query(value = "SELECT * FROM product WHERE CATEGORY = :category ORDER BY STATUS AND END_DATE DESC LIMIT :limit, :CONTENT_CNT", nativeQuery = true)
	...
	@Query(value = "SELECT P.* FROM product P LEFT JOIN watchlog W ON P.ID = W.PRODUCT_ID WHERE CATEGORY = :category GROUP BY P.ID ORDER BY STATUS AND COUNT(P.ID) DESC LIMIT :limit, :CONTENT_CNT", nativeQuery = true)
